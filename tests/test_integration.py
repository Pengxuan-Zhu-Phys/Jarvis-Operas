...
def test_build_register_dicts_returns_full_name_to_callable_only() -> None:
    ...
    _register(registry, "dmddxn", "LZSI2024", lambda x: x * 2.0)

    mapping = build_register_dicts(registry)
    assert callable(mapping["dmddxn.LZSI2024"])
    assert set(mapping.keys()) == {"dmddxn.LZSI2024"}
...
def test_build_sympy_dicts_for_namespace_function_parsing_and_eval() -> None:
    ...
    mapping = build_register_dicts(registry)
    func_locals, numeric_funcs = build_sympy_dicts(mapping, namespaces=["dmddxn"])

    expr = sp.sympify("dmddxn.LZSI2024(mchi)", locals=func_locals)
...
def test_refresh_sympy_dicts_if_global_registry_updates_module_level_dicts(monkeypatch) -> None:
    ...
    expr = sp.sympify("dmddxn.LZSI2024(mchi)", locals=integration.func_locals)
